...
def detect_shape(color_img):
    ...
    scaleX = .5
    scaleY = .5
    new_dims = (int(color_img.shape[1] * scaleX), int(color_img.shape[0] * scaleY))
    color_img = cv2.resize(color_img, new_dims)

    color_code = cv2.COLOR_BGR2GRAY
    color_img = cv2.cvtColor(color_img, color_code)

    color_img = cv2.GaussianBlur(color_img,ksize=(11,11),sigmaX=0)

    #color_img = cv2.adaptiveThreshold(img, maxValue=255, adaptiveMethod=cv2.ADAPTIVE_THRESH_GAUSSIAN_C, thresholdType=cv2.THRESH_BINARY_INV, blockSize=33, C=10)
    
    dum = cv2.threshold(color_img, 120, 255, cv2.THRESH_BINARY)
    color_img = dum[1]

    h, w = color_img.shape[:2]
    mask = np.zeros((h + 2, w + 2), np.uint8)
    dum = cv2.floodFill(color_img, mask, (0, 3), 255)
    color_img = dum[1]
    ...
    #     big_index = len(arr) - 2
    # print(big_index)        
    moments = cv2.moments(contours[big_index])
    if moments["m00"] == 0:
        return "idle"
    ...
    area = cv2.contourArea(contours[big_index])
    # print("contour: ", big_index)
    #print("centorid: ", mx, my)
    print(area)
    word = "idle"
    if (area > 4000) and (area < 11000):
        word = "stop"
    elif(area > 1500) and (area < 4000):
        if(mx > 110) and (mx < 155):
            word = "left"
        elif(mx >= 155) and (mx < 176):
            word = "straight"
        elif(mx >= 176) and (mx < 210):
            word = "right"
    else:
        word = "idle"
    ...
    # print(word)
    x,y,w,h = cv2.boundingRect(contours[big_index])
    x2 = int(x + w / 2)
    middy = ((x2, y), (x2, y + h))
    moment = (mx, my)

    #yay = part2_checkoff(formatted_img, contours, big_index, moment, middy, instruction)
